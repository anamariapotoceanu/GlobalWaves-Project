...
import app.user.NextPage;
import app.user.Notification;
import app.user.PreviousPage;
...
public final class CommandRunner {
     ...
     * @return Returns the message corresponding to the command.
     */
    ...
    public static ObjectNode getNotifications(final CommandInput commandInput) {
        UserAbstract currentUser = admin.getAbstractUser(commandInput.getUsername());
        ObjectNode objectNode = objectMapper.createObjectNode();
        objectNode.put("command", commandInput.getCommand());
        objectNode.put("user", commandInput.getUsername());
        objectNode.put("timestamp", commandInput.getTimestamp());
        ...
        for (Notification notification : currentUser.getNotifications()) {
            ...
            notificationsOutput.add(notificationNode);
        }
        currentUser.clearNotifications();
        objectNode.set("notifications", notificationsOutput);

        return objectNode;
    }

    /**
     * Ends the program and generates a summary.
     * @param commandInput The commandInput
     * @return A summary for the artist.
     */
    public static ObjectNode endProgram(final String commandInput) {
        ObjectMapper objectMapper = new ObjectMapper();
        ObjectNode objectNode = objectMapper.createObjectNode();
        ObjectNode resultNode = objectMapper.createObjectNode();

        List<Artist> artists = admin.getArtists();
        List<Artist> artistsAux = admin.artistsLoad();
        if (artistsAux != null && !artistsAux.isEmpty()) {
            artistsAux.sort(Comparator.comparing(Artist::getUsername));
            for (int i = 0; i < artistsAux.size(); i++) {
                Artist artist = artistsAux.get(i);
                ObjectNode artistNode = objectMapper.createObjectNode();
                artistNode.put("merchRevenue", artist.merchRevenue);
                artistNode.put("mostProfitableSong", artist.mostProfitableSong);
                artistNode.put("ranking", i + 1);
                artistNode.put("songRevenue", artist.songRevenue);

                resultNode.set(artist.getUsername(), artistNode);
            }
        }
        artists.sort(Comparator.comparingDouble(Artist::getTotalRevenue).reversed());

        for (int i = 0; i < artists.size(); i++) {
            Artist artist = artists.get(i);
            if (artist.getTotalRevenue() != 0.0 && artist.getBuyedMerch() != null) {
                ObjectNode artistNode = objectMapper.createObjectNode();
                artistNode.put("merchRevenue", artist.merchRevenue);
                artistNode.put("mostProfitableSong", artist.mostProfitableSong);
                artistNode.put("ranking", i + 1);
        ...
        objectNode.put("command", commandInput);
    ...
    /**
     * Purchases merchandise from an artist.
     * @param commandInput The commandInput.
     * @return Returns the message corresponding to the command.
     */
    ...
    /**
     * Displays all items purchased by the user.
     * @param commandInput The commandInput.
     * @return  Returns the message corresponding to the command.
     */
    public static ObjectNode seeMerch(final CommandInput commandInput) {
        List<String> merchandises = admin.seeMerch(commandInput);

        ObjectNode objectNode = objectMapper.createObjectNode();
        objectNode.put("command", commandInput.getCommand());
        objectNode.put("user", commandInput.getUsername());
        objectNode.put("timestamp", commandInput.getTimestamp());
        objectNode.put("result", objectMapper.valueToTree(merchandises));

        return objectNode;
    }

    /**
     * Update the recommendations for a user.
     * @param commandInput The commandInput
     * @return  Returns the message corresponding to the command.
     */
    public static ObjectNode updateRecommendations(final CommandInput commandInput) {
        String message = admin.updateRecommendations(commandInput, admin.getSongs());
        ObjectNode objectNode = objectMapper.createObjectNode();
        objectNode.put("command", commandInput.getCommand());
        objectNode.put("user", commandInput.getUsername());
        objectNode.put("timestamp", commandInput.getTimestamp());
    ...
    public static ObjectNode previousPage(final CommandInput commandInput) {
        PreviousPage previousPage = new PreviousPage(admin);
        String message = previousPage.execute(commandInput);
        ObjectNode objectNode = objectMapper.createObjectNode();
        objectNode.put("command", commandInput.getCommand());
        objectNode.put("user", commandInput.getUsername());
        objectNode.put("timestamp", commandInput.getTimestamp());
    ...
    public static ObjectNode nextPage(final CommandInput commandInput) {
        NextPage nextPage = new NextPage(admin);
        String message = nextPage.execute(commandInput);
        ObjectNode objectNode = objectMapper.createObjectNode();
        objectNode.put("command", commandInput.getCommand());
        objectNode.put("user", commandInput.getUsername());
        objectNode.put("timestamp", commandInput.getTimestamp());
    ...
    public static ObjectNode loadRecommendations(final CommandInput commandInput) {
        ...
        String message = admin.loadRecommendations(commandInput);
    ...
    /**
     * A statistics feature of the platform that provides
     * users with a personalized summary of their musical activity.
     * @param commandInput The commandInput
    ...
    public static ObjectNode wrapped(final CommandInput commandInput) {
        ...
        objectNode.put("user", commandInput.getUsername());
        objectNode.put("timestamp", commandInput.getTimestamp());
        admin.wrapped(commandInput);
        if (admin.getHosts().stream().anyMatch(host -> host.getUsername()
            ...
            if (admin.getHosts().stream().anyMatch(host -> host.getUsername()
                ...
                ObjectNode resultNode = objectMapper.createObjectNode();
                Host currentHost;
                currentHost = admin.getHosts().stream()
                        .filter(host -> host.getUsername().equals(commandInput.getUsername()))
                        .findFirst()
                        .orElse(null);
                resultNode.put("listeners", currentHost.getUserListeners().size());
                ObjectNode topEpisodes = objectMapper.createObjectNode();
                for (Episode episode : currentHost.getTopEpisodes()) {
                    topEpisodes.put(episode.getName(), episode.getListeners());
                }
        ...
        if (admin.getUsers().stream().anyMatch(user -> user.getUsername()
                .equals(commandInput.getUsername()))) {
                User user = admin.getUser(commandInput.getUsername());
                ObjectNode resultNode = objectMapper.createObjectNode();
                resultNode.set("topAlbums", user.getTopAlbums().isEmpty()
                        ? objectMapper.createObjectNode() : objectMapper
                        .valueToTree(user.getTopAlbums()));
                resultNode.set("topArtists", user.getTopAlbums().isEmpty()
                        ? objectMapper.createObjectNode() : objectMapper
                        .valueToTree(user.getTopArtists()));
                resultNode.set("topArtists", user.getTopArtists().isEmpty()
                        ? objectMapper.createObjectNode() : objectMapper
                        .valueToTree(user.getTopArtists()));
                ObjectNode topEpisodes = objectMapper.createObjectNode();
                for (Episode episode : user.getTopEpisodes()) {
                    topEpisodes.put(episode.getName(), episode.getListeners());
                }

                resultNode.set("topEpisodes", topEpisodes);
                resultNode.set("topGenres", user.getTopGenres().isEmpty()
                        ? objectMapper.createObjectNode() : objectMapper
                        .valueToTree(user.getTopGenres()));
                resultNode.set("topSongs", user
                        .getTopSongs().isEmpty()
                        ? objectMapper.createObjectNode() : objectMapper
                        .valueToTree(user.getTopSongs()));
